...
class LanaArxivImageProcessor(BaseImageProcessor):
    ...
    def preprocess(
        ...
        **kwargs: Any,
        ...
        if not valid_images(images):
            raise ValueError("LanaArxivImageProcessor expected a PIL image, numpy array, torch tensor, or list of images.")
        ...
        for image in images:
            ...
            if self.do_normalize:
                array = normalize(array, mean=self.image_mean, std=self.image_std, input_data_format=input_data_format)
            array = to_channel_dimension_format(array, data_format, input_channel_dim=input_data_format)
            pixel_values.append(np.asarray(array, dtype=np.float32))
